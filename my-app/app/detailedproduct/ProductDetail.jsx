...
import { AnimatedSection, AnimatedText, SlideInFromLeft, SlideInFromRight } from '../component/AnimatedSection';
import Image from 'next/image';
import Header from '../component/Header';
import Footer from '../component/Footer';
import { ALL_PRODUCTS } from '../component/Products';

const NAV_LINKS = ['About us','Manufacturing Facilities','Products','PharmaCovigilance','News & Blog','Careers','Contact us'];

const PRODUCT_STYLING = {
  'Pelton-C': { color: '#1a7f7a', bgGradient: 'linear-gradient(135deg, #e0f2f1 0%, #b2dfdb 100%)' },
  'Vonoglob 10mg': { color: '#3b5bdb', bgGradient: 'linear-gradient(135deg, #e8f0fe 0%, #c5cae9 100%)' },
  'Glomov 500mg/20mg': { color: '#111111', bgGradient: 'linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%)' },
  'Artinil-K 75mg': { color: '#1a7f7a', bgGradient: 'linear-gradient(135deg, #e0f7fa 0%, #b2ebf2 100%)' },
  'Nalbin 20mg Injection': { color: '#c0392b', bgGradient: 'linear-gradient(135deg, #fbe9e7 0%, #ffccbc 100%)' },
  'Anzonil 3mg': { color: '#555555', bgGradient: 'linear-gradient(135deg, #f3e5f5 0%, #e1bee7 100%)' },
  'Citolin 250mg Injection': { color: '#4f8566', bgGradient: 'linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%)' },
  'Piractim 1gm Injection': { color: '#406da5', bgGradient: 'linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%)' },
  'Tamsol-D': { color: '#147ea2', bgGradient: 'linear-gradient(135deg, #e0f7fa 0%, #b2ebf2 100%)' },
  'Norbac 1gm IM': { color: '#d41a21', bgGradient: 'linear-gradient(135deg, #ffebee 0%, #ffcdd2 100%)' },
};

const DEFAULT_STYLING = { color: '#ff6b6b', bgGradient: 'linear-gradient(135deg, #ffeaa7 0%, #fab1a0 100%)' };
...
const IMAGE_OVERRIDES = {
  ...
  'Citolin 250mg Injection': 'Citolin 250 inj.png',
...
export default function ProductDetail({ productName = 'Pelton-C', onBack }) {
  ...
  const product = productData ? {
    ...
    description: productData.description || `${productData.name} is a premium pharmaceutical product designed to deliver superior quality and efficacy. It contains ${productData.generic} in ${productData.form} form and is registered under number ${productData.reg}.`,
    img: productData.image ? productData.image : getGlobalPharmaImage(productData.name),
  ...
  const styles = {
    page: { 
      ...
      padding: screenSize.isMobile ? '20px' : screenSize.isTablet ? '40px 20px' : '80px 20px' 
    },
    container: { 
      display: 'flex', 
    ...
    imageBox: { 
      ...
      alignItems: 'center', 
      justifyContent: 'center', 
      minWidth: screenSize.isMobile ? '100%' : screenSize.isTablet ? '280px' : '320px', 
    ...
    productName: { 
      ...
      fontSize: screenSize.isMobile ? '28px' : screenSize.isTablet ? '32px' : '40px', 
      fontWeight: 700, 
      color: "#9d0b0f", 
      margin: 0 
    },
    description: { 
      fontSize: screenSize.isMobile ? '16px' : screenSize.isTablet ? '15px' : '17px', 
      color: '#333', 
      lineHeight: 1.7, 
      margin: 0 
    },
    ...
    footerText: { 
      fontSize: screenSize.isMobile ? '12px' : screenSize.isTablet ? '13px' : '16px', 
      color: '#555', 
      lineHeight: 1.7 
    }
  };
    ...
    <Header></Header>
      ...
      <div style={styles.page}>
        <div style={styles.container}>
          <button style={styles.mobileBackBtn} onClick={handleBack}>← Back to products</button>
          <SlideInFromLeft>
            <div style={styles.imageBox}>
              ...
              {product.img ? (
                <img 
                  src={product.img} 
                  alt={productName} 
                  style={{ width: '200%', height: '40%', maxWidth: '580px', maxHeight: '280px', objectFit: 'contain' }} 
                />
              ) : (
                <div style={{ background: product.bgGradient, borderRadius: '8px', padding: '20px', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <h1 style={{ fontSize: 72, color: product.color, opacity: 0.3 }}>{productName.slice(0, 1)}</h1>
                </div>
              )}
            </div>
          </SlideInFromLeft>
          <SlideInFromRight>
            <div style={styles.infoBox}>
              ...
              <AnimatedText>
                <p style={{color: "black", fontWeight:"bold"}} >Each tablet contains:</p>
              </AnimatedText>
              ...
              {product.packSize && (
                <AnimatedText delay={0.05}>
                  <p style={{ 
                    fontSize: screenSize.isMobile ? '16px' : screenSize.isTablet ? '18px' : '20px', 
                    color: '#9D0B0F', 
                    fontWeight: 600, 
                    margin: '4px 0 8px 0',
                  }}>
                   <span style={{
                    textDecoration: 'underline'}}>Pack Size: </span> 
                  <span style={{ color: '#000000', fontWeight: 700 , textDecoration: 'none'}}>{product.packSize}</span>
                  </p>
                </AnimatedText>
              )}
{/*               
